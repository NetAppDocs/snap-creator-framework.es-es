...
Por defecto, una operación de copia de seguridad fallará si una o más bases de datos se encuentran en un estado incoherente o dañado. Puedes forzar al complemento a continuar con las operaciones de copia de seguridad a pesar de los errores, como se describe en el paso link:task_using_the_gui_to_create_a_configuration_file.html["tarea que utiliza la gui para crear un archivo de configuración"].
...
El plug-in de Domino no puede funcionar con SNAP_TIMESTAMP_ONLY = N. SNAP_TIMESTAMP_ONLY debe ser Y.
 a| 
Para más información, consulta el paso link:task_using_the_gui_to_create_a_configuration_file.html["tarea_utilizar_la_interfaz_de_usuario_para_crear_un_archivo_de_configuración"].
