...
Ya deberías haber identificado el volumen changeinfo como un volumen del que hacer copia de seguridad cuando creaste el archivo de configuración. Para más información, consulta el paso link:task_using_the_gui_to_create_a_configuration_file.html["tarea que utiliza la gui para crear una configuración"].
